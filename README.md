...
저는 삼성전자에서 약 13년간 Smart TV 환경에서 콘텐츠 검색 서비스 서버 개발과 검색 품질 고도화를 담당해 왔습니다.

검색 시스템을 단순한 기능이 아닌 **사용자 경험을 결정하는 핵심 인터페이스**로 바라보며, 기술 도입 자체보다 “왜 필요한가, 실제 서비스에 어떻게 안착시킬 것인가”를 중요하게 생각합니다.

최근에는 LLM과 Vector 기반 검색 기술을 실험적 적용에 그치지 않고, **운영·성능 및 상황별 제약을 고려한 상용 검색 구조로 설계·확장**하는 데 집중하고 있습니다.
...
---
## 1. Smart TV 미들웨어 개발 (2013–2015)

### Work

- Smart TV 홈 화면(Partial UI)에 노출되는 파트너사 추천 콘텐츠 연동 미들웨어 N사, Y사등 콘텐츠 파트너사로부터 추천 콘텐츠를 주기적 스케줄링 방식으로 수신
- 수신된 콘텐츠를 TV 내에 저장하고 TV 앱이 안정적으로 조회/활용할 수 있는 구조 및 데몬 프로세스 구현

### Result

- 파트너 콘텐츠를 홈 화면에 안정적으로 노출할 수 있는 초기 미들웨어 구조 확보
- 이후 Smart TV 홈/검색 서비스 확장의 기반 마련

---


## 2. 검색 서비스 초기 구축 및 인프라 설계 (2016–2018)
...
## 4. 음성 검색 및 NER 기반 질의 이해 고도화 (2021–2023)
...
## 5. LLM 기반 메타데이터 증강 및 Semantic Search 고도화 (2024)
...
### Approach
- 검색 서버팀 리더로써 기술 아키텍처 설계 및 유관 부서 협업
...
## 6. AI Agent 기반 검색·추천 경험 (2025 – )

### Background

기존 TV 검색은 콘텐츠 결과 중심이었으나 LLM 기술 발전으로 사용자들의 기대가 변화하기 시작했습니다.

### Challenge

사용자가 검색·추천 결과와 그 근거를 함께 이해할 수 있도록 하는 새로운 탐색 경험 설계가 필요했습니다.
...
- Grounding과 Orchestration 병렬 처리 아키텍처 도입

### Result

- 스트리밍 응답 및 병렬 처리 도입으로 체감 응답 시간 평균 3초 이상 개선
- 검색·추천 결과의 이유를 설명하는 탐색 경험 제공
- LLM 기반 기능을 상용 Smart TV 검색 환경에 안정적으로 통합
